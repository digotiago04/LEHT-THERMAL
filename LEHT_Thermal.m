function LEHT_Thermal(k_m, k_i, frac, field, x_cut, y_cut)
...
TLEHT_Y = evaluate_temperature(x_cut*ones(size(yV)), yV, coef_plot, Hx, Hy);
TLEHT_X = evaluate_temperature(xH, y_cut*ones(size(xH)), coef_plot, Hx, Hy);
...
title(sprintf('Temperature field at $y_2 = %.2f$', y_cut), 'Interpreter', 'latex', 'FontSize', 16, 'FontWeight', 'bold');
end
...
ylabel('Temperature ($^\circ$C)', 'Interpreter', 'latex', 'FontSize', 16, 'FontWeight', 'bold');
title(sprintf('Temperature field at $y_1 = %.2f$', x_cut), 'Interpreter', 'latex', 'FontSize', 16, 'FontWeight', 'bold');
end
end
